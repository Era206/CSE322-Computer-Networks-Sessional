...
// //newly implemented to make tcp veno
	...
	if (tcph->seqno() > last_ack_) {
		...
		if(tcph->seqno() >= v_begseq_) {
			...
			if(rtt>0) {
				...
				if(cwnd_prev >= ssthresh_prev){
					if(delta_check < v_beta_ && delta_check > v_alpha_){
						if(new_actual>tht_prev){
							v_alpha_=v_alpha_+1;
							v_beta_=v_beta_+1;
						}
					}
					else if(delta_check<v_alpha_){
						if(v_alpha_>1){
							//  if(new_actual < tht_prev){
							// 	v_alpha_=v_alpha_-1;
							// 	//v_beta_=v_beta_-1;
					...
					else if(delta_check>v_beta_){
						if(v_alpha_>1){
							v_alpha_=v_alpha_-1;
							v_beta_=v_beta_-1;
						}
						// if(v_alpha_+1 < v_beta_){
						// 	//v_alpha_=v_alpha_-1;
						// 	v_beta_=v_beta_-1;
						// }
					}
